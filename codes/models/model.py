...
class SAIN(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        self.output, LR = self.net(x=self.input)  # output: 前三个维度是y_hat, 后9个维度是z；LR: 用于编码的Low-Resolution image，尺寸和y_hat相同

        zshape = self.output[:, 3:, :, :].shape # to be discarded during testing, but to be used to train GMM during training, (b,c*3,h//4,w//4)
        LR_ref = self.ref_L.detach()  # 二次下采样的低分辨率图片

        l_forw_fit1 = self.loss_forward(LR, LR_ref)  # lambda_fit_forw, LR corresponds to High-quality LR

        # backward upscaling
        LR = self.Quantization(LR)
        LR_ = self.diffcomp(LR)  # 可微分JPEG simulator, responding to y_prime
        ...
        l_back_rec = self.loss_backward(x_samples_recon, self.real_H) # lambda_rec_back
        ...
        l_rel = self.train_opt['lambda_fit_forw'] * self.Reconstruction_forw(self.output[:, :3, :, :], self.realcomp(self.Quantization(LR)))  # y_hat 与 真实编解码后的低分辨率重建图片
...
class INCA(BaseModel):
    def __init__(self, opt):
        super(INCA, self).__init__(opt)
        if opt['dist']:
            self.rank = torch.distributed.get_rank()
        else:
            self.rank = -1  # non dist training
    ...
    def optimize_parameters(self, step):
        ...
        LB = self.output[:,:3, :, :]
        B_ = torch.cat((LB, gaussian_scale * self.gmm_batch(zshape)), dim=1)
        # backward pass
        edited_I = self.net(x=B_, rev=True)
        # differential JPEG simulator
        edited_I_Q = self.Quantization(edited_I)
        recon_H = self.diffcomp(edited_I_Q)
        # loss
        # TODO: 设计更好的权重系数，按照每个loss的功能分组
        # recon_loss: 输入图片和JPEG simulator输出
        l_rec_all = self.loss_forward(recon_H, self.input) # lambda_rec_back
        # recon_loss: 输入图片和pre-edited input
        l_fit_wave = self.train_opt['lambda_fit_wave'] *self.loss_forward(recon_H, self.input) # lambda_rec_back
        # 低频子带和二次下采样图片
        l_fit_bicu = self.train_opt['lambda_fit_bicu'] * self.loss_forward(LB, self.ref_L)
        # 输出图片和真实JPEG, recon_loss
        l_real = self.train_opt['lambda_fit_real'] * self.Reconstruction_forw(recon_H, self.realcomp(self.Quantization(edited_I)))

        loss = l_rec_all + (l_fit_wave + l_fit_bicu + l_real) / 3 
        ...
        # set log
        self.log_dict['l_rec_all'] = l_rec_all.item()
        self.log_dict['l_fit_wave'] = l_fit_wave.item()
        self.log_dict['l_fit_bicu'] = l_fit_bicu.item()
        self.log_dict['l_real'] = l_real.item()
        mus, pis, logvars = map(lambda x: x.detach().cpu().numpy(), [self.net.module.gmm.mus, self.net.module.gmm.pis, self.net.module.gmm.logvars])
        for i, (mu, pi, logvar) in enumerate(zip(mus, pis, logvars)):
            self.log_dict[f'mu{i}'] = mu
            self.log_dict[f'pi{i}'] = pi
    ...
    def test(self):
        ...
        self.net.eval()
        with torch.no_grad():
            self.forw_L = self.Quantization(self.net(x=self.input)[:,:3,:,:])  # 低频子带
            y_forw = torch.cat((self.realcomp(self.forw_L), gaussian_scale * self.gmm_batch(zshape)), dim=1)
            self.fake_H = self.net(x=y_forw, rev=True) # 重建图片
    ...
    def downscale(self, HR_img):
        self.net.eval()
        with torch.no_grad():
            LR_img = self.Quantization(self.net(x=HR_img))  # 返回中间压缩特征
        self.net.train()
