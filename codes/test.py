...
if __name__ == "__main__":
    ...
    for test_loader in test_loaders:
        ...
        for data in tqdm(test_loader, total=len(test_loader)):
            ...
            visuals['Ed_input'] = back_L = model.upscale(forw_L, opt['scale'])
            if compress_quality == -1:
                visuals['SR'] = fake_H = model.upscale(forw_L, opt['scale'])
            else:
                # backward pass
                visuals['SR'] = REALCOMP(format=compress_format, quality=compress_quality)(back_L)
                # visuals['SR'] = fake_H = model.upscale(REALCOMP(format=compress_format, quality=compress_quality)(forw_L), opt['scale'])

            sr_img = util.tensor2img(visuals['SR'])  # uint8        JPEG编码重建
            srgt_img = util.tensor2img(visuals['GT'])  # uint8      原图
            lr_img = util.tensor2img(visuals['LR'])  # uint8        低频子带
            lrgt_img = util.tensor2img(visuals['LR_ref'])  # uint8  bicubic 下采样
            edited_img = util.tensor2img(visuals['Ed_input']) # uint8   预编辑后的图片，输入JPEG中
            ...
            if suffix:
                save_img_path = osp.join(dataset_dir, img_name + suffix + '_Edited.png')
            else:
                save_img_path = osp.join(dataset_dir, img_name + '_Edited.png')
            util.save_img(edited_img, save_img_path)
